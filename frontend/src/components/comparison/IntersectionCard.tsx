...
export default function IntersectionCard({ country1, country2 }: IntersectionCardProps) {
  const getLanguages = (c: Country) => Object.values(c.languages || {});
  const getCurrencies = (c: Country) => Object.values(c.currencies || {}).map((c) => c.name);
      ...
      <div className={styles.section}>
        ...
        <div className={styles.uniqueRow}>
          <div className={styles.uniqueColumn}>
            <span className={styles.uniqueLabel} style={{ color: '#00FFD1' }}>{country1.name.common}</span>
            {uniqueLang1.map((lang) => (
              <span key={lang} className={styles.uniqueTag}>{lang}</span>
            ))}
          </div>
          <div className={styles.uniqueColumn}>
            <span className={styles.uniqueLabel} style={{ color: '#2ff801' }}>{country2.name.common}</span>
        ...
        {sharedCurrencies.length > 0 ? (
          <div className={styles.shared}>
            {sharedCurrencies.map((curr) => (
              <span key={curr} className={styles.tag}>✓ {curr}</span>
            ))}
          </div>
        ) : (
          <div className={styles.noData}>NONE DETECTED</div>
        ...
        <div className={styles.uniqueRow}>
          <div className={styles.uniqueColumn}>
            ...
            {uniqueCurr1.map((curr) => (
              <span key={curr} className={styles.uniqueTag}>{curr}</span>
            ))}
          </div>
          <div className={styles.uniqueColumn}>
            <span className={styles.uniqueLabel} style={{ color: '#2ff801' }}>{country2.name.common}</span>
        ...
        {sharedTimezones.length > 0 ? (
          <div className={styles.shared}>
            {sharedTimezones.map((tz) => (
              <span key={tz} className={styles.tag}>✓ {tz}</span>
            ))}
          </div>
        ) : (
          <div className={styles.noData}>NONE DETECTED</div>
        ...
        <div className={styles.uniqueRow}>
          <div className={styles.uniqueColumn}>
            ...
            {uniqueTz1.slice(0, 3).map((tz) => (
              <span key={tz} className={styles.uniqueTag}>{tz}</span>
            ))}
          </div>
          <div className={styles.uniqueColumn}>
            <span className={styles.uniqueLabel} style={{ color: '#2ff801' }}>{country2.name.common}</span>
        ...
        {sharedBorders.length > 0 ? (
          <div className={styles.shared}>
            {sharedBorders.map((border) => (
              <span key={border} className={styles.tag}>✓ {border}</span>
            ))}
          </div>
        ) : (
          <div className={styles.noData}>NONE DETECTED</div>
